...
Armies v3.0 is a single compiled Go binary. There is no Python runtime, no virtual environment, no Docker image to pull. You download or build the binary, put it somewhere in your `PATH`, and `armies` works. The binary embeds the bundled example profiles directly -- when you run `armies seed`, those profiles are unpacked from inside the binary itself into your `~/.armies/profiles/` directory.

This matters operationally. There are no dependency conflicts. There is no interpreter version mismatch. There is nothing to activate or deactivate. On any machine where the binary runs, armies works identically. The binary is statically compiled with CGO disabled, which means it has no shared library dependencies -- it runs on any Linux or macOS system without requiring specific libc versions or other runtime libraries.
...
The practical result: the Armies engine can manage dozens of profiles on disk without stuffing every spawn prompt with irrelevant content. Each deployment gets exactly the context the agent needs for the assigned mission.
...
The engine searches `~/.armies/profiles/` before the bundled profile pack. Your Grace Hopper, with her two hundred deployment entries and three Medals of Honor, takes precedence over any bundled Grace Hopper. The engine never reaches the bundled version when a private version exists.
